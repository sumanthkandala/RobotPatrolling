...
global C1,C2,C3,C4,theta1,theta2
C1 = 1.0
C2 = 0.0
C3 = 0.0
C4 = 0.0
...
def DTAG_Reward(current,target,nodes):
	...
	return theta1*nodes.nodes[target,1] + theta2*((curr_x-target_x)**2+(curr_y-target_y)**2)**0.5
...
def DTAG(departed, vehicles, nodes, routes_list, path_list, shortest_path):
	...
	for veh_id in departed:
		...
		if curr_vehicle.curr_edge == curr_vehicle.dest_edge:
			...
			for route in routes_list:
				if target == None or (reward < DTAG_Reward(curr_target,int(route[route.find("to")+2:-1]),nodes)):
					end_edge = route[1:len(route)-1]
					target = int(route[route.find("to")+2:-1])
					reward = DTAG_Reward(curr_target,int(route[route.find("to")+2:-1]),nodes)
			curr_vehicle.changeTarget(end_edge) #Assign new path
			...
			for clash_veh_id in departed:
				...
				if target == clash_target and clash_veh_id != veh_id:
					...
					for route in routes_list:
						if clash_target == None or clash_reward<DTAG_Reward(clash_curr_location,int(route[route.find("to")+2:-1]),nodes):
							clash_dest_edge = route[1:len(route)-1]
							clash_target = int(route[route.find("to")+2:-1])
							clash_reward = DTAG_Reward(clash_curr_location,int(route[route.find("to")+2:-1]),nodes)
