...
class simulation:
	def __init__(self):
		...
		self.heatmap = []
		self.size = 100
		self.shortest_path = self.size*(len(self.node.nodes))*np.ones((len(self.node.nodes),len(self.node.nodes)))
		#Generate routes
		def gen_route():
		...
		def getpaths():
			min_path = self.node.nodes[:,3]
			max_path = max(min_path[np.where(self.node.nodes[:,2] == 0)])
			min_path = min(min_path[np.where(self.node.nodes[:,2] == 0)])
		...
		def setup():
			...
			# for node in (self.node).interest_nodes:
			# 	for lane in self.routes:
			# 		if lane.find(":") == -1 and lane.find("to"+str(node)+".") != -1:
			# 			lane = lane[1:-1]
			# 			veh = vehicle.addVehicle(lane,self.speed,len(self.vehicles),(self.node).num_nodes)
			# 			self.vehicles.append(veh)
			# 			break
			print((self.node).num_nodes)
			veh = vehicle.addVehicle("1to0",(self.node).num_nodes,len(self.vehicles),self.speed)
			self.vehicles.append(veh)
			# veh = vehicle.addVehicle("3to6",(self.node).num_nodes,len(self.vehicles),self.speed)
			# self.vehicles.append(veh)
			# veh = vehicle.addVehicle("5to2",(self.node).num_nodes,len(self.vehicles),self.speed)
			# self.vehicles.append(veh)
		...
		def step():
			if traci.simulation.getMinExpectedNumber()>0:
			    ...
			    self.node.nodes = patrolling.algo(self.departed,self.vehicles,self.node,self.routes,self.paths,self.shortest_path)
...
def simulate():
	...
	simulator.setup()
	max_freq = str(int(max(simulator.node.nodes[np.where(simulator.node.nodes[:,2] == 0),3][0])))
	#print("max_freq",max_freq)

	with open("idleness_"+str(len(simulator.vehicles))+"_"+max_freq+".csv","w") as file:
		with open("time_imp_"+str(len(simulator.vehicles))+"_"+max_freq+".csv","w") as imp_file:
			...
			while simulator.curr_time<simulator.total_time:
				...
				for node in range(len(simulator.node.nodes)):
					row = simulator.curr_time,node,simulator.node.nodes[node,1]
					writer.writerow(row)
				for node in simulator.node.interest_nodes:
					row = simulator.curr_time,node,simulator.node.nodes[node,3]-simulator.node.nodes[node,1]
					writer_imp.writerow(row)	
				print(np.mean(simulator.node.nodes[:,1]))


	#print(simulator.node.visited)
	with open("time_patrolling_"+str(len(simulator.vehicles))+"_"+max_freq+".csv","w") as file:
		writer = csv.writer(file)
		node_id = 0
		for count in simulator.node.visited:
			row = node_id,count
			node_id = node_id + 1
			writer.writerow(row)
	print(simulator.heatmap.transpose())
